...
export async function generateFallbackPDF(
  ...
  console.warn('🔄 폴백 PDF 생성 시작:', errorInfo.message);
  
  try {
    // 서버 사이드 PDF 생성 함수 사용
    const { generateHandwritingPDF } = await import('./pdf-server-utils');
    
    const fallbackOptions = {
      font: originalOptions.font,
      fontSize: originalOptions.fontSize,
      customTexts: originalOptions.customTexts?.slice(0, 1) // 첫 번째 텍스트만 사용
    };
    
    const pdfBytes = await generateHandwritingPDF(fallbackOptions);
    console.log('✅ 폴백 PDF 생성 성공');
    
    return pdfBytes;
    
  } catch (fallbackError) {
    console.error('💥 폴백 PDF 생성도 실패:', fallbackError);
    throw new Error('PDF 생성에 완전히 실패했습니다. 잠시 후 다시 시도해주세요.');
  }
}
